...
function emptyResourceLoader(systemPrompt) {
  return {
...
export async function runTrial(spec) {
  ...
  try {
    ...
    const loader = emptyResourceLoader(provider.systemPrompt);
    ...
    try {
      await session.prompt(fixturePrompt(spec.fixture, copied), { images });
