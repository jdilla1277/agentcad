...
import { loadCadbenchEnvironment } from "./env.js";
...
function usage() {
  ...
  cadbench candidate add NAME --provider agentcad-cli --agentcad-bin PATH [--skill-file PATH] [--source-type TYPE] [--source VALUE]
...
async function initWorkspace(cwd, options) {
  ...
  const additions = [".env", "runs/", "experiments/", "candidate-resources/"].filter((entry) => !ignored.has(entry));
  if (additions.length) {
...
async function addCandidate(cwd, name, options) {
  ...
  if (options.skill_file) {
    const sourcePath = path.resolve(cwd, options.skill_file);
    const content = await readFile(sourcePath);
    const skillDir = path.join(cwd, "candidate-resources", name, "skills", "agentcad");
    const snapshotPath = path.join(skillDir, "SKILL.md");
...
export async function main(argv = process.argv.slice(2), dependencies = {}) {
  ...
  if (command === "env") {
    const state = dependencies.environmentState ?? {
      files: [],
      providers: {},
    };
...
if (isMainModule()) {
  ...
  const execution = loadCadbenchEnvironment({ cwd: process.cwd(), env: process.env })
    .then((environmentState) => (
      shouldUseKeepAwake(argv)
        ? runWithKeepAwake(argv)
        : main(argv, { environmentState })
    ));
