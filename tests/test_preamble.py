...
def test_bare_script_no_imports(runner, isolated_dir):
    """Script with explicit cadquery import still uses the cq preamble's
    helper injection. Post-Phase-6, no-import scripts route to b3d (which
    doesn't inject ``cq``); this test specifically exercises the cq
    preamble path via explicit ``import cadquery as cq``."""
...
def test_error_line_numbers_reasonable(runner, isolated_dir):
    ...
    _init_project(runner)
    # Line 1: import (route to cq runner), Line 2: good, Line 3: bad.
    _write_script(isolated_dir, content=(
        'import cadquery as cq\n'
        'result = cq.Workplane("XY").box(10, 10, 10)\n'
...
def test_docs_preamble_section(runner):
    """``agentcad docs preamble --runtime cadquery`` lists the cq
    pre-injected names. Pinned cq since post-Phase-6 the default
    routes to b3d's preamble, which lists different names."""
