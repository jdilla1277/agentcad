...
<style>
  ...
  #part-controls .explode-block { padding: 10px 12px; border-bottom: 1px solid rgba(0,0,0,0.08); }
  #part-controls .explode-head { display: flex; justify-content: space-between; font-size: 11px; color: #27313f; margin-bottom: 6px; }
  #part-controls .explode-head span:last-child { font-weight: 700; }
  #part-controls .explode-block input[type=range] { width: 100%; margin: 0; }
  #spec-panel {
...
<div id="part-controls">
  ...
  <div class="explode-block" id="explode-block" style="display:none;">
    <div class="explode-head"><span>Explode</span><span id="explode-value">0%</span></div>
    <input type="range" id="explode-slider" min="0" max="200" step="5" value="0">
  </div>
  <div class="rows" id="part-control-rows"></div>
  <div class="foot">Focus or isolate by stable part id. This state can be generated by agents with agentcad parts view; agentcad explode seeds the slider.</div>
</div>
<aside id="spec-panel">
  <div class="head">
    <div class="verdict">
...
let explodeFactor = (PART_REVIEW && typeof PART_REVIEW.explode === 'number')
  ? Math.max(0, PART_REVIEW.explode)
  : 0;
const partExplodeDirs = new Map();
...
function setupAgentHandoffPanel() {
  ...
  if (typeof PART_REVIEW.explode === 'number') {
    appendAgentHandoffRow(details, 'Explode', Math.round(PART_REVIEW.explode * 100) + '%');
  }
...
// ---- Exploded view ----
// Each part gets a radial direction (part center - model center) computed
// once from the loaded meshes. The slider offsets mesh positions by
// direction * factor, so factor 1.0 (100%) doubles part separation —
// matching the geometry `agentcad explode` renders to PNG.
function setupExplodeVectors() {
  partExplodeDirs.clear();
  if (partObjects.size < 2) return;
  const modelBox = new THREE.Box3();
  for (const mesh of allPartMeshes()) modelBox.expandByObject(mesh);
  if (modelBox.isEmpty()) return;
  const modelCenter = modelBox.getCenter(new THREE.Vector3());
  for (const [partId, meshes] of partObjects) {
    const box = new THREE.Box3();
    for (const mesh of meshes) box.expandByObject(mesh);
    if (box.isEmpty()) continue;
    partExplodeDirs.set(partId, box.getCenter(new THREE.Vector3()).sub(modelCenter));
  }
}

function applyExplode() {
  for (const mesh of allPartMeshes()) {
    const dir = partExplodeDirs.get(mesh.userData.partId);
    if (!dir || !mesh.userData.basePosition) continue;
    mesh.position.copy(mesh.userData.basePosition).addScaledVector(dir, explodeFactor);
  }
}

function setExplodeFactor(value) {
  explodeFactor = Math.max(0, value);
  applyExplode();
  const pct = Math.round(explodeFactor * 100);
  const label = document.getElementById('explode-value');
...
function setupExplodeControls() {
  const block = document.getElementById('explode-block');
  const slider = document.getElementById('explode-slider');
  if (!block || !slider) return;
  if (partExplodeDirs.size < 2) { block.style.display = 'none'; return; }
  block.style.display = 'block';
  const pct = Math.round(explodeFactor * 100);
  if (pct > Number(slider.max)) slider.max = String(pct);
  slider.value = pct;
  document.getElementById('explode-value').textContent = pct + '%';
  slider.addEventListener('input', () => setExplodeFactor(Number(slider.value) / 100));
}
...
Promise.all([
  ...
  if (explodeFactor > 0) {
    applyExplode();
    // Exploded parts extend past the assembled bounds; refit around them.
    for (const mesh of allPartMeshes()) combinedBox.expandByObject(mesh);
  }
  fitCamera();
  applyPartState();
