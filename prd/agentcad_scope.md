...
agentcad is a command-line tool that enables AI coding agents (like Claude Code) to create, iterate on, and inspect 3D CAD models programmatically. The agent writes CadQuery scripts; agentcad handles geometry, export, rendering, and a structured version history. Humans review and approve the output before any downstream handoff.

The core loop is:

1. Human describes a design problem to Claude Code in natural language
2. Claude Code writes a CadQuery script and runs agentcad
3. agentcad produces a STEP file, 3D renders, and a structured JSON log
...
- **Agent discoverability over convention.** The tool does not assume a CLAUDE.md or any host project structure. It ships with `--help`, `agentcad docs`, and `agentcad context` for provider-agnostic discoverability.
...
Initializes a new agentcad project in the current directory. Creates the project folder structure and top-level manifest.

```bash
agentcad init
agentcad init --name enclosure
```
...
```bash
agentcad run script.py --output v1
agentcad run script.py --output v1 --formats step,glb,obj,png
agentcad run script.py --input v1/output.step --output v2_mounting_holes --formats step,png-iso,png-top
```
...
```bash
agentcad render v1/output.step --view iso
agentcad render v1/output.step --view top --zoom 2
agentcad render v1/output.step --view "45,30" --focus "10,10,5" --name hole_check
```
...
```bash
agentcad docs
agentcad docs render
agentcad docs schema
```
...
Every agentcad invocation returns JSON to stdout. The schema is consistent regardless of command.
...
```json
{
  ...
  "status": "success",
  "instruction": "agentcad run script.py --input v1/output.step --output v2_mounting_holes",
...
```
project/
  agentcad.json                  ← top-level manifest
  v1/
    script.py
    output.step
    output.glb
...
The agent reads `agentcad.json` first to orient itself, then dives into individual `meta.json` files only if it needs detail.
...
- Linux support
- Template / starter commands (`agentcad create --template enclosure`) — planned for v1 once scripting workflow is validated
- Helper library of parametric primitives — planned for v1
- Natural language input to agentcad directly — Claude Code is the NL layer; agentcad takes structured commands only
- FEA, CFD, DFM, or any analysis beyond geometry and rendering
- STEP file as input to `agentcad run` — v2 feature, enables importing manufacturer parts and existing designs
- PyPI / Homebrew distribution
- CLAUDE.md generation — `agentcad context` is the provider-agnostic alternative
...
- **LLaVA-3D** — multi-view image approach with 3D positional embeddings. Validates the multi-view render strategy agentcad uses.
...
| Tool | Notes |
|------|-------|
| CadQuery | Python CAD scripting library wrapping Open CASCADE. The geometry layer for agentcad. [github.com/CadQuery/cadquery](https://github.com/CadQuery/cadquery) |
...
| pythonOCC | Python bindings for Open CASCADE. The rendering layer for agentcad v0. |
...
Native 3D understanding in LLMs — where a model reads an OBJ or STEP file the way it reads an image — is probably 2-3 years from being reliable enough to use in an engineering context. The multi-view render approach agentcad uses is not a limitation of the current design. It is the right design for now, and it positions the tool to benefit directly as 3D-native models mature.
